...
func TestUserRepository_Get(t *testing.T) {
	WithTx(t, func(ctx context.Context, tx postgres.DBTX) {
		...
		require.Equal(t, users[0].ID, got.ID)
		require.Equal(t, users[0].Email, got.Email)
		require.Equal(t, users[0].PasswordHash, got.PasswordHash)
		require.Equal(t, users[0].Role, got.Role)

		got, err = userRepo.Get(ctx, domain.User{Email: users[1].Email})
		require.NoError(t, err)
		require.NotNil(t, got)
		require.Equal(t, users[1].ID, got.ID)
		require.Equal(t, users[1].Email, got.Email)
		require.Equal(t, users[1].PasswordHash, got.PasswordHash)
		require.Equal(t, users[1].Role, got.Role)

		got, err = userRepo.Get(ctx, domain.User{Role: domain.EmployeeRole})
		require.NoError(t, err)
		require.NotNil(t, got)
		require.Equal(t, users[0].ID, got.ID)
		require.Equal(t, domain.EmployeeRole, got.Role)
