...
func TestReceptionStatusRepository_Get(t *testing.T) {
	WithTx(t, func(ctx context.Context, tx infra.DBTX) {
		repo := repo.NewReceptionStatusRepository(tx)
		...
		err := repo.CreateBatch(ctx, statuses)
		require.NoError(t, err)

		got, err := repo.Get(ctx, domain.ReceptionStatus{ID: statuses[0].ID})
		require.NoError(t, err)
		require.NotNil(t, got)
		assert.Equal(t, statuses[0].ID, got.ID)
		assert.Equal(t, statuses[0].Name, got.Name)

		got, err = repo.Get(ctx, domain.ReceptionStatus{Name: statuses[1].Name})
		require.NoError(t, err)
		require.NotNil(t, got)
		assert.Equal(t, statuses[1].ID, got.ID)
		assert.Equal(t, statuses[1].Name, got.Name)

		_, err = repo.Get(ctx, domain.ReceptionStatus{ID: uuid.New()})
		assert.ErrorIs(t, err, infra.ErrNotFound)

		_, err = repo.Get(ctx, domain.ReceptionStatus{Name: "NonExistent"})
		assert.ErrorIs(t, err, infra.ErrNotFound)
	})
}
...
func TestReceptionStatusRepository_CreateBatch(t *testing.T) {
	WithTx(t, func(ctx context.Context, tx infra.DBTX) {
		...
		gotOpen, err := repo.Get(ctx, domain.ReceptionStatus{Name: "Open"})
		require.NoError(t, err)
		assert.Equal(t, "Open", string(gotOpen.Name))
		assert.Equal(t, statuses[0].ID, gotOpen.ID)

		gotClose, err := repo.Get(ctx, domain.ReceptionStatus{Name: "Close"})
		require.NoError(t, err)
		...
		err = repo.CreateBatch(ctx, duplicate)
		require.NoError(t, err)

		// Проверяем, что количество записей не увеличилось
		// проверяем существующие по именам
		gotOpen2, err := repo.Get(ctx, domain.ReceptionStatus{Name: "Open"})
		require.NoError(t, err)
		assert.Equal(t, gotOpen.ID, gotOpen2.ID)

		gotClose2, err := repo.Get(ctx, domain.ReceptionStatus{Name: "Close"})
		require.NoError(t, err)
